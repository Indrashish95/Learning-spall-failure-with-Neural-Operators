...
import numpy as np
from UFNO_utils import Net3d, set_seed
...
model.load_state_dict(torch.load(
    main_dir + f"saved_model/UFNO3d-1shot-25g1-{resp_type}_{split}.pth",map_location=device
))
model.eval()
...
with torch.no_grad():
    for i, (x_test, y_test) in enumerate(test_dataloader):
        ...
        if i ==0: 
            ...
            plt.colorbar()
            plt.savefig(main_dir+"figures/20g_2_error.jpg",dpi = 300)
            plt.close()
            plt.imshow(test_data[0,0,-1,:,:].cpu().numpy(),cmap = "coolwarm")
            plt.colorbar()
            plt.savefig(main_dir+"figures/20g_2_micro.jpg",dpi = 300)
            plt.close()
            plt.imshow(test_data[0,1,-1,:,:].cpu().numpy(),cmap = "coolwarm")
            plt.colorbar()
            plt.savefig(main_dir+"figures/20g_2_true.jpg",dpi = 300)
            plt.close()
            ...
            plt.savefig(main_dir+"figures/20g_2_freesurf_vel.jpg",dpi = 300)
            plt.close()
...
# Save
np.save(
    main_dir + f"test_data/test_UFNO_all20g_{resp_type}_{split}.npy",
    test_data.cpu().numpy()
)
