...
Ndata = 800 # number of training samples  # train + val = 750 
...
modes = 16
width = 40
model = Net3d(modes, modes, modes, width).to(device)
...
for ep in range(epochs):
    ...
    np.save(main_dir+"train_data/"+f"train_UFNO_micro25g_{resp_type}",x.cpu().numpy())   
    np.save(main_dir+"train_data/"+f"train_UFNO_true25g_{resp_type}",y.cpu().numpy())  
    np.save(main_dir+"train_data/"+f"train_UFNO_pred25g_{resp_type}",out.detach().cpu().numpy())  
    ...
    if val_err[ep] < best_loss:
            ...
            torch.save(model.state_dict(), main_dir+"saved_model/"+f"UFNO3d-1shot-25g1-{resp_type}_{split}.pth")
            eps = 0.1
            ...
            error = abs(y_val[0,:,:,-1,0].cpu().numpy()-out_val[0,:,:,-1].detach().cpu().numpy())

            plot(x_val[0,:,:,-1,0].cpu().numpy(),y_val[0,:,:,-1,0].cpu().numpy(),out_val[0,:,:,-1].detach().cpu().numpy(),error,main_dir,titles = ep,filename=f"Epoch_{ep}_UFNOimpact_val_{resp_type}_{split}.png")

            np.save(main_dir+"test_data/"+f"val_UFNO_micro25g_{resp_type}_{split}",x_val.cpu().numpy())   
            np.save(main_dir+"test_data/"+f"val_UFNO_true25g_{resp_type}_{split}",y_val.cpu().numpy())  
            np.save(main_dir+"test_data/"+f"val_UFNO_pred25g_{resp_type}_{split}",out_val.detach().cpu().numpy()) 
    
    np.savetxt(main_dir+f"UFNO3d-train_loss_{resp_type}_{split}",train_err)
    np.savetxt(main_dir+f"UFNO3d-test_loss_{resp_type}_{split}",val_err)
    ...
    plt.savefig(main_dir+f"UFNO3d_loss25g_{resp_type}_{split}.png")
    plt.close()
...
plot(x_test[0,:,:,-1,0].cpu().numpy(),y_test[0,:,:,-1,0].cpu().numpy(),out_test[0,:,:,-1].detach().cpu().numpy(),
     error,main_dir,titles = ep,filename=f"UFNOimpacttest_{resp_type}_{split}.png")
np.save(main_dir+"test_data/"+f"test_UFNO_micro25g_{resp_type}_{split}",x_test.cpu().numpy())   
np.save(main_dir+"test_data/"+f"test_UFNO_true25g_{resp_type}_{split}",y_test.cpu().numpy())  
np.save(main_dir+"test_data/"+f"test_UFNO_pred25g_{resp_type}_{split}",out_test.detach().cpu().numpy()) 
